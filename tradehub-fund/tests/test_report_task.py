...
class TestReportTask:
    ...
    def test_skips_user_with_report_disabled(self):
        User = get_user_model()
        user = User.objects.create_user(username='user', password='pass1')
        AIConfig.objects.create(user=user, api_endpoint='https://a.com', api_key='k', model_name='gpt-4')
    ...
    def test_generates_reports(self):
        ...
        Position.objects.create(account=child, fund=fund, holding_share='100',
                               holding_cost='120.00', holding_nav='1.2')

        import datetime as dt
        monday = dt.date(2026, 6, 8)  # 周一

        with patch('api.tasks.requests.post') as mock_post, \
             patch('api.tasks.date') as mock_date:
