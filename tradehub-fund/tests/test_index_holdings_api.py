...
class TestIndexHoldingsAPI:
    ...
    @pytest.fixture
    def linked_etf_fund(self):
        from api.models import Fund
        Fund.objects.create(
            fund_code='560550',
            fund_name='广发中证全指电力ETF',
            fund_type='股票指数型',
        )
        return Fund.objects.create(
            fund_code='016186',
            fund_name='广发中证全指电力ETF发起式联接C',
            fund_type=None,
        )
    ...
    @patch('api.sources.eastmoney.EastMoneySource.fetch_index_holdings')
    def test_holdings_realtime_linked_fund_fallback_to_etf_code(self, mock_fetch, client, linked_etf_fund, mock_holdings):
        """联接基金优先 fallback 到匹配到的场内 ETF 代码"""
        mock_fetch.side_effect = [
            [],
            mock_holdings,
        ]

        response = client.get(f'/api/funds/{linked_etf_fund.fund_code}/holdings-realtime/')

        assert response.status_code == 200
        assert response.data['target_code'] == '560550'
        assert len(response.data['holdings']) == 2
