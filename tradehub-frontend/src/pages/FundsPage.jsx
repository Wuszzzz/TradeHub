...
const FundsPage = () => {
  ...
  const loadFunds = async (searchValue = search, pageNum = page) => {
    ...
    try {
      ...
      await loadSectors(response.data.results);
    } catch (error) {
  ...
  const loadSectors = async (fundList) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      onHeaderCell: (column) => ({
        width: column.width,
        onResize: handleResize('sectors'),
      }),
      render: (code) => {
        const sectors = sectorMap[code] || [];
        return sectors.length > 0 ? (
          <Space wrap size={[4, 4]}>
            {sectors.map((item) => (
              <Text key={item.name} style={{ fontSize: 12 }}>
                {item.name} {Number(item.ratio).toFixed(1)}%
              </Text>
            ))}
          </Space>
        ) : '-';
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      {(sectorMap[fund.fund_code] || []).length > 0 && (
                        <div style={{ fontSize: 12, color: '#667085', marginTop: 4 }}>
                          所属板块: {(sectorMap[fund.fund_code] || []).map((item) => `${item.name} ${Number(item.ratio).toFixed(1)}%`).join(' / ')}
                        </div>
                      )}
                    </div>
