...
def _tencent_json(url):
    ...
    data = resp.json()
    if data.get('code') != 0:
        raise ValueError(data.get('msg') or '腾讯基金接口返回异常')
    return data.get('data') or {}


def _tencent_rank_board(board_code, sort_type='price', direct='down', offset=0, count=20):
    resp = requests.get(
    ...
    data = resp.json()
    if data.get('code') != 0:
        raise ValueError(data.get('msg') or '腾讯板块排行接口返回异常')
    return data.get('data') or {}


def _tencent_rank_indices():
    resp = requests.get(
    ...
    data = resp.json()
    if data.get('code') != 0:
        raise ValueError(data.get('msg') or '腾讯股指排行接口返回异常')
    return data.get('data') or {}


def _qq_text(url, referer='https://stockapp.finance.qq.com/', encoding=None):
    resp = requests.get(
        url,
        headers={
            'User-Agent': 'Mozilla/5.0',
...
def _tencent_rank_us(board_type='US_cdr', sort_type='price', direct='down', offset=0, count=20):
    resp = requests.get(
    ...
    data = resp.json()
    if data.get('code') != 0:
        raise ValueError(data.get('msg') or '腾讯美股排行接口返回异常')
    return data.get('data') or {}


def _normalize_market_symbol(raw_code):
...
def _market_api_json(path, params=None):
    ...
    if not payload.get('ok'):
        raise ValueError(payload.get('error') or '行情接口返回异常')
...
@api_view(['GET'])
@permission_classes([AllowAny])
def tencent_fund_detail(request, symbol):
    """
    聚合腾讯基金详情页关键模块，便于前端复刻页面。
    symbol 示例：sh501019 / sz159611 / 501019
    """
    try:
        market_symbol = _normalize_market_symbol(symbol)
    except ValueError:
        return Response({'error': '无效 symbol'}, status=400)

    base = 'https://web.ifzq.gtimg.cn/fund/newfund'
    asset_base = 'https://zxg.txfund.com/ifzqgtimg/appstock/fund/baseInfo/asset'

    try:
        rank_info = _tencent_json(f'{base}/fundBase/getRankInfo?symbol={market_symbol}')
        notices = _tencent_json(f'{base}/fundNotice/getNotice?app=web&symbol={market_symbol}&page=1&limit=9')
        same_type = _tencent_json(f'{base}/fundBase/getSameLxFundList?type=1&symbol={market_symbol}')
        same_series = _tencent_json(f'{base}/fundBase/getSameLxFundList?type=2&symbol={market_symbol}')
        profit = _tencent_json(f'{base}/fundProfit/getProfitHistory?app=web&symbol={market_symbol}')
        asset = _tencent_json(f'{asset_base}?code={market_symbol}')
    except Exception as e:
        return Response({'error': str(e)}, status=502)

    notice_data = ((notices or {}).get(market_symbol) or {}).get('data') or []
    profit_data = ((profit or {}).get(market_symbol) or {}).get('data') or []
    ...
    return Response({
        ...
        'asset': asset,
        'notices': notice_data,
        'same_type_funds': (same_type or {}).get('list') or [],
        'same_series_funds': (same_series or {}).get('list') or [],
...
@api_view(['GET'])
@permission_classes([AllowAny])
def tencent_market_board(request):
    board_code = request.GET.get('board_code', 'aStock')
    sort_type = request.GET.get('sort_type', 'price')
    direct = request.GET.get('direct', 'down')
    offset = int(request.GET.get('offset', 0))
    count = int(request.GET.get('count', 20))
    try:
        data = _tencent_rank_board(board_code, sort_type, direct, offset, count)
...
@api_view(['GET'])
@permission_classes([AllowAny])
def tencent_market_indices(request):
    try:
        data = _tencent_rank_indices()
        return Response(data)
    except Exception as e:
        return Response({'error': str(e)}, status=502)
...
@api_view(['GET'])
@permission_classes([AllowAny])
def tencent_market_us(request):
    board_type = request.GET.get('board_type', 'US_cdr')
    sort_type = request.GET.get('sort_type', 'price')
    direct = request.GET.get('direct', 'down')
    offset = int(request.GET.get('offset', 0))
    count = int(request.GET.get('count', 20))
    try:
        data = _tencent_rank_us(board_type, sort_type, direct, offset, count)
        return Response(data)
    except Exception as e:
        return Response({'error': str(e)}, status=502)
...
@api_view(['GET'])
@permission_classes([AllowAny])
def tencent_market_fx(request):
    symbols = request.GET.get('symbols', 'whUSDCNY,whEURCNY,whHKDCNY,whJPYCNY').split(',')
    text = _qq_text(f'https://qt.gtimg.cn/q={",".join(symbols)}', encoding='gbk')
    rows = []
    for line in text.strip().split(';'):
        ...
        rows.append({
            ...
            'high': parts[8],
            'low': parts[9],
            'trade_date': parts[-1] if parts else '',
        })
...
@api_view(['GET'])
@permission_classes([AllowAny])
def tencent_market_futures(request):
    ...
    try:
        data = _tencent_json('https://proxy.finance.qq.com/ifzqgtimg/appstock/app/rank/worldCommodities')
        normalized = {}
        for key, rows in (data or {}).items():
            normalized[key] = [
                {
                    'qtcode': item.get('qtcode'),
                    'code': item.get('code'),
                    'name': item.get('name'),
                    'zxj': item.get('zxj'),
                    'zdf': item.get('zdf'),
        ...
        target_key = alias_map.get(category, None)
        rows = [item for items in normalized.values() for item in items] if target_key is None else normalized.get(target_key, [])
        return Response({
            'category': category,
            'groups': normalized,
            'rows': rows,
...
@api_view(['GET'])
@permission_classes([AllowAny])
def fund_market_kline(request, fund_code):
    try:
        market_symbol = _normalize_market_symbol(fund_code)
    except ValueError:
        return Response({'error': '无效基金代码'}, status=400)

    period = request.GET.get('period', 'day')
    adjust = request.GET.get('adjust', 'qfq')
    limit = int(request.GET.get('limit', 120))
    preferred_source = request.GET.get('source', 'eastmoney')
    source_paths = {
        'eastmoney': '/api/v1/eastmoney/kline',
        'tencent': '/api/v1/tencent/kline',
        'sohu': '/api/v1/sohu/kline',
    }
    order = [preferred_source] + [item for item in ('eastmoney', 'tencent', 'sohu') if item != preferred_source]
    last_error = None

    for source in order:
        try:
            data = _market_api_json(
                source_paths[source],
                params={
                    'symbol': market_symbol,
                    'period': period,
            ...
            if not rows:
                raise ValueError(f'{source} 未返回 K 线数据')
            return Response({
                'symbol': market_symbol,
                'source': source,
                'period': period,
...
def build_report_context(user, period='weekly'):
    ...
    # 账户总览
    accounts = Account.objects.filter(user=user, parent=None).prefetch_related('children__positions__fund')
    ...
    top_str = '\n'.join([f'{i+1}. {name}: ¥{pnl:.2f}({rate})' for i, (name, pnl, rate) in enumerate(top)])
    worst_str = '\n'.join([f'{i+1}. {name}: ¥{pnl:.2f}({rate})' for i, (name, pnl, rate) in enumerate(worst)])
...
def ai_report_preview(request):
    ...
    ai_config = AIConfig.objects.filter(user=request.user).first()
    if not ai_config:
        return Response({'error': '未配置AI接口，请先在设置中配置'},
