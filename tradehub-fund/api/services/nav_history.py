...
def sync_nav_history(
    ...
    if not force and not start_date:
        last_record = FundNavHistory.objects.filter(fund=fund).order_by('-nav_date').first()
        if last_record:
    ...
    if not nav_data:
        ...
        with transaction.atomic():
            for item in nav_data:
                ...
                latest_record = FundNavHistory.objects.get(fund=fund, nav_date=item['nav_date'])
                from .daily_fact import upsert_daily_fact_from_nav
                upsert_daily_fact_from_nav(fund, latest_record)
                if created:
                    count += 1
